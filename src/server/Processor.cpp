...
void Processor::InstantMessage() {
  int clientSock = socket->getSockFd(); 
  ...
  int err;
  while ((err=hasAuthenticated(clientSock, userName))!=0) {
      if(err==-1)
          return ;
...
void Processor::broadcastMsg(string userName, string msg, bool isConnected) {
...
int Processor::hasAuthenticated(int clientSock, string &userName) {
...
bool Processor::SendMessage(int HostSock, string msg) {
...
string Processor::GetMessage(int HostSock, int messageLength) {
...
long Processor::GetInteger(int HostSock) {
...
bool Processor::SendInteger(int HostSock, int hostInt) {
...
void Processor::SaveMsg(string msg, string userFrom) {
...
void Processor::processMsg(string &msg, string &cmdName, string &userTo) {
...
string Processor::GrabTime(string userName) {
...
string Processor::GrabUsers(string userName) {
...
bool Processor::loginUser (string username, string password) {
  User newUser;
...
void Processor::addToUsersList (User newUser) {
  pthread_mutex_lock(&UserListLock);
  UsersList.insert (make_pair<string, User> (newUser.username, newUser));
  pthread_mutex_unlock(&UserListLock);
}

bool Processor::doesUserExist (string username) {
  pthread_mutex_lock(&UserListLock);
...
bool Processor::isUserConnected (User newUser) {
  pthread_mutex_lock(&UserListLock);
...
void Processor::setUserConnected (User newUser) {
  pthread_mutex_lock(&UserListLock);
...
void Processor::setUserDisconnected (string username) {
  pthread_mutex_lock(&UserListLock);
  tr1::unordered_map<string, User>::iterator got = UsersList.find (username);
  if (got == UsersList.end() ) {
    pthread_mutex_unlock(&UserListLock);
